...
public class banco_sincronizar4_palabraReservadasynchronized
{
//la palabrea reservada Synchronized sirve para utilizar dos metodos void wait() y void notifyAll() hace lo mismo
//que los metodos await() y signalAll() de la interfaz Condition
	public static void main(String[] args)
	{
		banco4 mibanco=new banco4();
		
		for(int i=0;i<100;i++) 
		{
			Runnable r=new ejecucionTransferencias4(mibanco,i,2000);
...
class banco4 
{
	...
	//siqueremos usar la plabra reservada Sychrinized para usar los metodos parecidos a los ya mencionados
	//arriba del todo devemos usarlo en el metdo donde ocurre la tranferencia y poner esa palabra en
	//este metodo despues del public de esta forma le vamos a decir a este metodo que tiene
	//que estar sincronizado y si vamos a usar esta palavra reservada ya nos sobran los cierres
	//explicitos (lock y unlock) y  las condiciones de bloqueo (await y singnalAll) por lo que 
	//el finally tampoco sirve que esta desbloquenado el el codigo con unlock() y el try tampoco
	//nos vale esta palabra reservada sirve solo para bloquear hilos si cumple una condicion
	//que esta dentro del while si quicieramos usar esta palabra reservada para establecer mas
	//condiciones tendriamos que usar lock() unlock() await() singanlAll() ya que porque los 
	//metodo de la clase Object con la palabra reservada synchronized no sirven para mas de una
	//condicion
	public synchronized void transferir4(int cuentaorigen,int cuentadestino,double cantidadtransferir) throws InterruptedException 
	{
		//cierrebanco.lock();
		//try 
		//{
			...
			while(cuentas[cuentaorigen]<cantidadtransferir==true) 
			{
				...
				//metodo que ereda de la clase cosmica Object y hace lo mismo que el metodo
				//await() de la interfaz Condition
				wait();
			}
			...
			notifyAll();//de la clase Object que hace lo mismo que signalAll
		//}	
		/*finally 
		{
			cierrebanco.unlock();
...
{	
	...
	public ejecucionTransferencias4(banco4 b,int origen,double max)
	{
	...
	public void run() 
	{
		...
		while(true) 
		{
			...
			try 
			{
				banco.transferir4(Origen, paraLaCuentaDestino, cantidad);
			} 
